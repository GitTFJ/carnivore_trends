    ...
    for(b in seq(0.02,0.22,0.04)){
      for(c in seq(2,20,2)){
        for(d in c(-1,1)){
          ...
          index = 1:20
          tryCatch((
          trend = coef(gls(log(base_df) ~ index,
                              correlation = corCAR1(form = ~index), 
                           control = list(singular.ok = TRUE)))[2]
          ...
          tryCatch((
          trend = coef(gls(log(abundance_tr) ~ year,
                           correlation = corCAR1(form = ~year), control = list(singular.ok = TRUE), data = model_df[c(min(samp):max(samp)),]))[2]
          ), error = function(e) { skip_to_next <<- TRUE})
if(skip_to_next) { next }  
          trend = trend*d
          tryCatch((
          estimate = coef(gls(log(abundance_est) ~ year, 
                             correlation = corCAR1(form = ~year), control = list(singular.ok = TRUE), data = model_df[samp,]))[2]
          ), error = function(e) { skip_to_next <<- TRUE})
if(skip_to_next) { next }  
          estimate = estimate*d
          tmp_df = data.frame(
...
ggsave("Results/example_simultating_weights.png", width = 5, height = 5)
...
ggsave("Results/simulated_trend_dist.png", width = 8, height = 3)
...
plot(m1)
hist(residuals(m1))
saveRDS(m1, "Data/Analysis/ErrorModel.rds")
...
ggsave("Results/simulated_effects.png", width = 8, height = 3)
n