...
LPI = read.csv("Data/Trends/lpi_master.csv", 
               row.names = NULL)
...
Area = read.csv("Data/Trends/Area(save).csv")
Coords = left_join(Coords[,c(5,4,3,1,2)], Area[,c(1,5)])
ID = read.csv("Data/Trends/ID(save).csv")
...
ggsave("Results/SamplingPoins.png", width = 9, height = 5
       )
