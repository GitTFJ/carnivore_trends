...
wrmse_vec = wrmse_vec[wrmse_vec < quantile(wrmse_vec, 0.99)] #Remove highest 1% of values which were clear outliers


rmse_plt = ggplot() +
  geom_density(aes(x = wrmse_vec), colour = "grey", fill = "light grey", alpha = 0.4) +
  labs(x = "Weighted RMSE") +
  coord_cartesian(xlim = c(17.2,18.5)) +
...
jpeg("Results/model_fit.jpeg", width = 9, height = 3, units = "in", res = 300)
...
ggsave("Results/rvi_coefs.png", width = 14, height = 7.5)
...
tmp.r = sum.ribbons
prc_plt = ggplot() +
  geom_ribbon(data = tmp.r, aes(x = trans, ymin = lci95, ymax = lci80), fill = "#782B9D", alpha = 0.2) +
  ...
  geom_ribbon(data = tmp.r, aes(x = trans, ymin = uci80, ymax = uci95), fill = "#782B9D", alpha = 0.2) +
...
ar_plt = ggplot() +
  ...
  labs(x = "Population area (km2)",
       y = " ") +
  scale_x_log10(breaks = c(100, 10000, 1000000), labels = c("100", "10,000  ", "1,000,000       "), limits = c(20,1400000), expand=c(0,0)) +
...
var.beta1 = merged.chains[ , grep(Dictionary[which(Dictionary$JagsTerm == "exh"),]$variable, colnames(merged.chains), fixed = T)]
var.beta2 = merged.chains[ , grep(Dictionary[which(Dictionary$JagsTerm == "pr"),]$variable, colnames(merged.chains), fixed = T)]
...
var.beta = var.beta[which(var.delta == 1)]
var.beta1 = var.beta1[which(var.delta == 1)]
var.beta2 = var.beta2[which(var.delta == 1)]
...
tmp.r = sum.ribbons
ec_pr_plt = ggplot() +
  geom_ribbon(data = tmp.r[which(tmp.r$group == 1.75),], aes(x = trans, ymin = lci95, ymax = lci80), fill = "#782B9D", alpha = 0.2) +
  ...
  geom_ribbon(data = tmp.r[which(tmp.r$group == 1.75),], aes(x = trans, ymin = uci80, ymax = uci95), fill = "#782B9D", alpha = 0.2) +
  geom_line(data = tmp.r[which(tmp.r$group == 1.75),], aes(x = trans, y = median), linetype = "dashed", size = 1.2) +
  geom_ribbon(data = tmp.r[which(tmp.r$group == -0.71),], aes(x = trans, ymin = lci95, ymax = lci80), fill = "#ffaa3b", alpha = 0.2) +
  ...
  geom_ribbon(data = tmp.r[which(tmp.r$group == -0.71),], aes(x = trans, ymin = uci80, ymax = uci95), fill = "#ffaa3b", alpha = 0.2) +
  ...
  scale_colour_manual(values = c("#e38100", "#4B2991"), name = "Protected area\ncoverage:") +
  labs(x = "Change in extreme heat",
       y = " ") +
  scale_x_continuous(breaks = c(0,1,2), expand=c(0,0)) +
  coord_cartesian(ylim = c(-1, 5), xlim = c(-0.1, 2.2)) +
  theme_classic() +
...
hdc_plt = ggplot() +
  ...
  labs(x = "Change in Human development (%)",
       y = "Annual rate\nof change (%)") +
  scale_x_continuous(breaks = c(0,0.5,1,1.5,2), expand=c(0,0)) +
  coord_cartesian(ylim = c(-1,3), xlim = c(0,2.1)) +
  theme_classic() +
  theme(axis.title.x = element_text(face = "bold"),
        axis.title.y = element_text(face = "bold"))
hdc_plt
...
alpha = alpha_raw
var.beta = merged.chains[ , grep(Dictionary[which(Dictionary$JagsTerm == "drc"),]$variable, colnames(merged.chains), fixed = T)]
var.delta = merged.chains[ , grep(gsub("beta", "delta", Dictionary[which(Dictionary$JagsTerm == "drc"),]$variable), colnames(merged.chains), fixed = T)]
alpha = alpha_raw[which(var.delta == 1)]
var.beta = var.beta[which(var.delta == 1)]
var = seq(-10, 10, length.out = 100)
comb.resp = NULL
...
trans.rib = ((sum.ribbons$scaled*sd((TrendsTrim_lag10$DroughtC), na.rm = T)) + mean((TrendsTrim_lag10$DroughtC), na.rm = T))
sum.ribbons$trans = as.vector(trans.rib)
...
drc_plt = ggplot() +
  ...
  labs(x = " Change in drought",
       y = "Annual rate\nof change (%)") +
  scale_x_continuous(breaks = c(0,0.5,1,1.5), expand=c(0,0)) +
  coord_cartesian(ylim = c(-1, 4), xlim = c(0,1.6)) +
  theme_classic() +
  theme(axis.title.x = element_text(face = "bold"),
        axis.title.y = element_text(face = "bold"))
...
jpeg("Results/coef_plot.jpeg", width = 11.6, height = 8, units = "in", res = 300)
  ...
  ggarrange(
    ...
    ggarrange(
      ggarrange(
        ...
        drc_plt,
        ec_pr_plt,
        labels = paste0(letters[seq(from = 2, to = 5)],","), 
        ncol = 2, 
        nrow = 2),
      hdc_plt,
      labels = c("", "f,"),
