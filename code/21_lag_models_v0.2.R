...
#source("code/Background/full_wg_aV1.7.R")
source("code/Background/full_wg_a_lag10_V1.7.R")
source("code/Background/full_wg_a_lag0_V1.7.R")

full_wg_a0 = readRDS("Data/Analysis/full_wg_a_lag0.rds")
full_wg_a5 = readRDS("Data/Analysis/full_wg_a.rds")
full_wg_a10 = readRDS("Data/Analysis/full_wg_a_lag10.rds")

mod_list = list(
  full_wg_a10, full_wg_a5, full_wg_a0
)
...
for(a in 1:length(mod_list)){
  if(a == 1){
    wt = jagsdata_lag10$wg_a
    mod_name = "10 years"
    ln = length(jagsdata_lag10$pt)
    cou = jagsdata_lag10$cou_n
    reg = jagsdata_lag10$reg_n
    spe = jagsdata_lag10$spec_n
    gen = jagsdata_lag10$gen_n
  } else if(a == 2){
    ...
    wt = jagsdata_lag0$wg_a
    mod_name = "0 years"
    ln = length(jagsdata_lag0$pt)
    cou = jagsdata_lag0$cou_n
    reg = jagsdata_lag0$reg_n
    spe = jagsdata_lag0$spec_n
    gen = jagsdata_lag0$gen_n
  }
  mod.mcmc = as.mcmc(mod_list[[a]])
...
df = round(df,3)
write.csv(df, "Results/lag_fit.csv")
...
beta_df$model = factor(beta_df$model, levels = c("0 years", "5 years", "10 years"))
...
ggsave("Results/lag_fit.png", width = 12, height = 7)
