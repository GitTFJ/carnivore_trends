 ...
 GDP = read.csv("Data/WG/GlobalGDPerCapita.csv")
 GDP <- GDP %>% gather(Year, GDP, c(X1960:X2018))
 GDP$GDP = as.numeric(GDP$GDP)
 colnames(GDP)[1] = "CountryCode"
 AllGovernance = left_join(AllGovernance, GDP)

 HDI = read.csv("Data/HDI/HDI.csv") #Merge Country name with ISO 3 and link
 ...
 Corruption = read.csv("Data/WG/Corruption.csv")
 ...
 GovEffect = read.csv("Data/WG/GovernmentEffectiveness.csv")
 ...
 PolStab = read.csv("Data/WG/PoliticalStabilityAndAbsenceOfViolence.csv")
 ...
 RegQual = read.csv("Data/WG/RegulatoryQuality.csv")
 ...
 RuleOfLaw = read.csv("Data/WG/RuleOfLaw.csv")
 ...
 VoiceAndAcc = read.csv("Data/WG/VoiceAndAccountability.csv")
...
Conflicts = read.csv("Data/Conflicts/Conflicts.csv") #Link via ISO 3
Conflicts = Conflicts %>%
...
saveRDS(CountryNum, "/Data/WG/CountryNum.csv")
...
saveRDS(GovernanceList, "Data/WG/ImputedGovernance.rds")
GovernanceList = readRDS("Data/WG/ImputedGovernance.rds")
png("Results/GovernanceConvergence_mean.png", width = 1200, height = 1500, res = 200)
...
saveRDS(Governance, "Data/WG/GovernanceFormatted.rds")
