...
PA = readRDS("Data/ProtectedAreas/PA_simple.rds")
...
for(a in 1960:2020){
  ...
  saveRDS(fr, paste("Data/ProtectedAreas/ProtectedAreaRaster_",a,".rds",sep = ""))
}
...
Rast = readRDS(paste("Data/ProtectedAreas/ProtectedAreaRaster_",LastEndYear,".rds",sep = ""))
for(a in 1:nrow(TempTrends)){
  id = TempTrends$ID[a]
  message(a)
  EndYear = TempTrends$Study_year_end[a]
  if(EndYear > LastEndYear){
    Rast = readRDS(paste("Data/ProtectedAreas/ProtectedAreaRaster_",EndYear,".rds",sep = ""))
...
saveRDS(CombDF, "Data/ProtectedAreas/ProtectedAreaEstimates.rds")
